...
const CourseDetails = ({ params }: { params: { courseId: string } }) => {
   ...
   const { addToCart }=useCartStore();
    ...
    const handelPayment = () => {
        if(token){
            buyCourse(token,[courseId],user,navigate.push);
        }
        else{
            navigate.push('/login');
        }
    ...
    useEffect(() => {
        if(courseDetail?.ratingAndReviews?.length > 0){
            ...
            console.log("getCourseDetails -> count", (count));
            }
    ...
    useEffect (() => {
    if(courseDetail){
        const Enrolled = courseDetail?.studentsEnrolled?.find((student:any) => student === user?._id);
                ...
                <div className='relative block max-h-[30rem] lg:hidden'>
                    ...
                    <div className='flex w-full flex-col gap-4 border-y border-y-richblack-500 py-4 lg:hidden'>
                        <p className='space-x-3 pb-4 text-3xl font-semibold text-richblack-5'>
                            ...
                            <>
                            {
                                alreadyEnrolled ? <button onClick={()=>{navigate.push("/dashboard/enrolled-courses")}}  className='yellowButton'>Go to Course</button> : <button onClick={handelPayment} className='yellowButton'>Buy Now</button>
                            }
                            {
                                alreadyEnrolled ? (<div></div>) : 
                                (
                                    cart?.find((item) => item?._id === courseDetail?._id) ?
                                    (<button onClick={()=>{navigate.push("/dashboard/cart")}} className='blackButton text-richblack-5'>Go to Cart</button>) :
                        ...
                        <div className='px-4'>
                            ...
                            <div className=''>
                                ...
                                <div className='flex flex-col gap-1 text-sm text-caribbeangreen-100'>
                                    {
                                        JSON.parse(courseDetail?.instructions).map((item:any,index:any) => (
                    ...
                    <div className='max-w-[830px] '>
                        <div className='flex flex-col gap-3'>
                            ...
                            <div className='flex flex-wrap justify-between gap-2'>
                                ...
                                <span>{courseDetail?.courseContent?.reduce((acc:any, item:any) => acc + item?.subSection?.length, 0)} Lecture(s)</span>
                                </div>
                        ...
                        <div className='py-4'>
                            {
                                courseDetail?.courseContent?.map((item:any, index:any) => (
                                    <details key={index} className=' border border-solid border-richblack-600 bg-richblack-700 text-richblack-5 detailanimatation'>
                                        ...
                                        <div className='mt-5'>
                                            {
                                                item?.subSection?.map((subItem:any, subIndex:any) => (
                    ...
                    <div className='my-8 border border-richblack-600 p-3 md:p-8'>
                        ...
                        <div className='mt-5'>
                            <div className='flex items-center gap-4'>
                                ...
                                {
                                    courseDetail?.ratingAndReviews?.map((item:any, index:any) => (
