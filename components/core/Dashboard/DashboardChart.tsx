...
const DashboardChart = ({details,currentChart}:any) => {
    ...
    const StudentsData = {
        labels: details?.map((course:any) => course?.courseName),
        datasets: [
            {
                label: '# of Students',
                data: details?.map((course:any) => course?.totalStudents),
    ...
    const RevenueData = {
        ...
        datasets: [
            {
                label: '# of ₹',
                data: details?.map((course:any) => course?.totalRevenue),
                backgroundColor: randomColor(details?.length),
                borderColor: randomColor(details?.length),
                borderWidth: 1,
            },
