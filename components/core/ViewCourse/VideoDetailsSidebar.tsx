...
const VideoDetailsSidebar = ({setReviewModal}:{setReviewModal:any}) => {
  ...
  return (
    ...
    <div className={ `${showSidebar?"h-0 w-0":"h-[calc(100vh-3.5rem)] w-[320px]"} transition-all duration-700 z-20 relative offSidebar1`}>
      <div className={`${showSidebar?"hidden":""} transition-all origin-right duration-500 flex h-[calc(100vh-3.5rem)] w-[320px] max-w-[350px] flex-col border-r-[1px] border-r-richblack-700 bg-[#0a0a0a] offSidebar2`}>
                ...
                </summary>
                {
                  section?.subSection.map((subSection:any, index:any) => (
                    <div  key={subSection?._id} className='transition-[height] duration-500 ease-in-out'>
                      <div onClick={()=>{
                        setShowSidebar(true);
                        navigate.push(`/view-course/${courseId}/${section?._id}/${subSection?._id}`);
